            ...
            <div class="comments-page-header">
                <div>
                    <h2 class="comments-page-title" style="margin: 0 0 4px 0; font-size: 1.25rem; font-weight: 800; color: #0f172a; border: none; padding: 0; display: flex; align-items: center; gap: 8px;">
                        <i class="bi bi-chat-left-text-fill" style="color: var(--primary);"></i> Comments &amp; Feedback
                    </h2>
                    <p style="margin: 0; color: #64748b; font-size: 0.82rem;">Manage profile comments, reply to fans, or moderate messages.</p>
                </div>
            ...
            <div class="comments-layout-grid">
                ...
                <div class="comments-sidebar-stack">
                    ...
                    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 14px; padding: 16px;">
                        <h4 style="margin: 0 0 12px 0; font-size: 0.9rem; font-weight: 800; color: #0f172a; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; display: flex; align-items: center; gap: 6px;">
                            <i class="bi bi-bar-chart-fill" style="color: #6366f1;"></i> Overview
                        </h4>
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <span style="font-size: 0.82rem; color: #64748b; font-weight: 600;">Total Received</span>
                                <span style="font-size: 0.9rem; font-weight: 800; color: #0f172a;">{{ $totalComments }}</span>
                            </div>
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <span style="font-size: 0.82rem; color: #64748b; font-weight: 600;">Direct Reviews</span>
                                <span style="font-size: 0.9rem; font-weight: 800; color: #6366f1;">{{ $totalTopLevel }}</span>
                            </div>
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <span style="font-size: 0.82rem; color: #64748b; font-weight: 600;">Replies Posted</span>
                    ...
                    <div style="background: linear-gradient(135deg, rgba(99,102,241,0.06) 0%, rgba(79,70,229,0.1) 100%); border: 1px solid rgba(99,102,241,0.2); border-radius: 14px; padding: 16px;">
                        <h4 style="margin: 0 0 8px 0; font-size: 0.88rem; font-weight: 800; color: #4338ca; display: flex; align-items: center; gap: 6px;">
                            <i class="bi bi-shield-check" style="font-size: 1.05rem;"></i> Moderation Control
                        </h4>
                        <p style="margin: 0 0 10px 0; font-size: 0.78rem; color: #475569; line-height: 1.45;">
                            Delete comments or replies that violate rules or contain inappropriate content.
                        </p>
                        <a href="{{ route('profile', auth()->id()) }}" target="_blank" style="display: inline-flex; align-items: center; gap: 5px; font-size: 0.78rem; font-weight: 700; color: #6366f1; text-decoration: none;">
                            View Public Profile <i class="bi bi-arrow-right"></i>
                        </a>
                    </div>
                ...
                <div style="min-width: 0;">
                    <h3 style="font-size: 0.98rem; font-weight: 800; color: #0f172a; margin-bottom: 14px; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; display: flex; align-items: center; justify-content: space-between;">
                        ...
                        <span style="font-size: 0.78rem; font-weight: 600; color: #64748b;">{{ $comments->count() }} items</span>
                    </h3>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 14px;">
                                ...
                                <div style="flex-grow: 1; min-width: 0;">
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; flex-wrap: wrap; gap: 6px;">
                                        ...
                                        <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                                            ...
                                            <!-- Delete Button -->
                                            <form action="{{ route('talent.comment.delete', $cmt->id) }}" method="POST" onsubmit="return confirm('Delete this comment?');" style="margin: 0;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="comment-action-btn" style="padding: 4px 9px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; background: rgba(239,68,68,0.1); color: #ef4444; border: 1px solid rgba(239,68,68,0.2); cursor: pointer; display: inline-flex; align-items: center; gap: 3px;" title="Delete this comment">
                                ...
                                <form action="{{ route('talent.comment', auth()->id()) }}" method="POST">
                                    ...
                                    <div style="display: flex; gap: 8px; align-items: flex-start;">
                                        ...
                                        <div style="flex-grow: 1;">
                                            <textarea name="comment" rows="2" class="form-control" placeholder="Write response..." required style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 10px; font-size: 0.84rem; line-height: 1.4; background: #f8fafc; margin-bottom: 8px;"></textarea>
                                            <div style="display: flex; justify-content: flex-end; gap: 6px;">
                                                <button type="button" onclick="toggleDashboardReplyBox({{ $cmt->id }})" style="padding: 5px 12px; border-radius: 6px; font-size: 0.78rem; font-weight: 600; background: #f1f5f9; color: #475569; border: none; cursor: pointer;">Cancel</button>
                                    ...
                                    <div style="flex-grow: 1; min-width: 0;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 3px; flex-wrap: wrap; gap: 4px;">
                                            ...
                                            <div style="display: flex; align-items: center; gap: 6px;">
                                                ...
                                                <form action="{{ route('talent.comment.delete', $reply->id) }}" method="POST" onsubmit="return confirm('Delete this reply?');" style="margin: 0;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" style="padding: 2px 6px; border-radius: 5px; font-size: 0.70rem; font-weight: 700; background: rgba(239,68,68,0.1); color: #ef4444; border: 1px solid rgba(239,68,68,0.2); cursor: pointer; display: inline-flex; align-items: center; gap: 2px;" title="Delete this reply">
                                                        <i class="bi bi-trash-fill"></i> Delete
                                                    </button>
                                                </form>
                                            </div>
                        ...
                        <div style="padding: 40px 15px; text-align: center; background: #f8fafc; border-radius: 14px; border: 1px dashed #cbd5e1;">
                            ...
                            <p style="margin: 0; color: #64748b; font-weight: 600; font-size: 0.88rem;">No comments received yet.</p>
                        </div>
                        @endforelse
                    </div>
                </div>
